...
import { getEditableComponents, getEditableElements } from './utils'
 ...
 * Components tab shows available components, Elements tab shows available elements
 */
export const PreviewModal: React.FC<PreviewModalProps> = ({
  ...
  const handleElementSelect = (elementKey: string) => {
    onSelectComponent(elementKey)
    onClose()
  }
  ...
  return (
    ...
    >
      {/* Isolated InlineEditProvider with empty templateContent to show original texts */}
      <InlineEditProvider
        templatePath=""
        initialContent={{}}
        initialComponents={[]}
      ...
      >
        <div className="relative bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            ...
            {activeTab === 'elements' && (
              <div className="flex flex-col gap-6">
                {editableElements.map(([key, element]) => (
                  <ElementPreview
                    key={key}
                    elementKey={key}
                    element={element}
                ...
                {editableElements.length === 0 && (
                  <div className="text-center py-12 text-gray-500">
                    No editable elements available
                  </div>
                )}
              </div>
            )}
